...
def stitch_XSH_spectra(waves, fluxs, errors, bpmaps, scale=None):
    ...
    # Get uvb offset
    p0 = [1e-15, -0.5]
    UVB_mask = (waves[0][UVB] > 5000) & (waves[0][UVB] < 5500)
    popt_UVB, pcuv = optimize.curve_fit(pow, waves[0][UVB][UVB_mask], fluxs[0][UVB][UVB_mask], sigma=errors[0][UVB][UVB_mask], p0=p0, maxfev=5000)
    VIS_mask = (waves[1][VIS] > 6000) & (waves[1][VIS] < 6500)
    popt_VIS, pcuv = optimize.curve_fit(pow, waves[1][VIS][VIS_mask], fluxs[1][VIS][VIS_mask], sigma=errors[1][VIS][VIS_mask], p0=p0, maxfev=5000)
    off_UVB = pow(waves[1][overlapUVBVIS[1]], *popt_VIS)[0] / pow(waves[0][overlapUVBVIS[0]], *popt_UVB)[0]

    # Get nir offset
    p0 = [1e-15, -0.5]
    VIS_mask = (waves[1][VIS] > 8000) & (waves[1][VIS] < 9000)
    popt_VIS, pcuv = optimize.curve_fit(pow, waves[1][VIS][VIS_mask], fluxs[1][VIS][VIS_mask], sigma=errors[1][VIS][VIS_mask], p0=p0, maxfev=5000)
    NIR_mask = (waves[2][NIR] > 10000) & (waves[2][NIR] < 13000)
    popt_NIR, pcuv = optimize.curve_fit(pow, waves[2][NIR][NIR_mask], fluxs[2][NIR][NIR_mask], sigma=errors[2][NIR][NIR_mask], p0=p0, maxfev=5000)
    off_NIR = pow(waves[2][overlapVISNIR[1]], *popt_NIR)[0] / pow(waves[1][overlapVISNIR[0]], *popt_VIS)[0]


    print(off_UVB, off_NIR)
    # Apply correction
    if scale:
        fluxs[0] *= off_UVB
        fluxs[2] /= off_NIR
