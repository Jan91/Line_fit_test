...
def main():
    burst = "GRB121024A"
    z_guess = 2.3
    data = np.genfromtxt("../data/"+burst+"/stitched_spectrum_bin10.dat")
    ...
    p.add_many(('amp_pow',            -8, True, -np.inf, 0),
               ('slope_pow',       -2.38, True, -5, 0),
               ('N',                  20, True, 18, 24),
               ('sigma',             200, True, 0),
               ('z',               z_guess, True, z_guess - 0.1, z_guess + 0.1),
               ('ebv',               1.0, True, 0),
               ('f',               0.416, False),
    ...
    pl.xlim(1216 * (1+mi.params['z'].value) - 200, 1216 * (1+mi.params['z'].value) + 200)
    pl.ylim(-1e-18, 1.5e-16)
    pl.savefig("../figs/"+burst+"DLA_fit_zoom.pdf")
    pl.xlim(3200, 10000)
    pl.ylim(-1e-18, 1.5e-16)
    pl.savefig("../figs/"+burst+"DLA_fit.pdf")
    pl.clf()
    ...
    fig.savefig("../figs/"+burst+"line-time.pdf")
    ...
    np.savetxt("../data/"+burst+"dla_fitresults.dat", data, header="names value +error -error", fmt = ['%16s', '%1.5e', '%1.5e', '%1.5e'])


    import corner
    corner.corner(res.flatchain, labels=res.var_names, truths=list(res.params.valuesdict().values()))
    pl.savefig("../figs/"+burst+"Cornerplot.pdf", clobber=True)
